...
wx.ready(function () {
	...
	var userId =getQueryString('userId');//获取url链接参数 
	var detailGoodsId =getQueryString('goodsId');//获取url链接参数
	if(!detailGoodsId||detailGoodsId==null){
    ...
    var onMenu={
		title: $(".detail_sp .goodsname").html(), // 分享标题
		desc: $(".detail_sp .saleSpots").html(), // 分享描述
		link: 'https://m.mcyou.net/mid_detail.html?goodsId='+detailGoodsId+"&userId="+userId, // 分享链接
		imgUrl:$(".swiper-slide-active img").attr("src"), // 分享图标
	} 
	  wx.checkJsApi({
            jsApiList: [
                'getLocation',
