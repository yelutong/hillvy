(function(root){
    ...
    var createElement = function(handler){
        ...
        this.UPRender(this.options.dataSource,this.options.index,this.options.kUP.kUPCELLHEIGHT);
    ...
    pickerview.prototype.UPRender = function(dataSource,index,lineHeight){
        if(!index){
            index =0;
        }
        if(Array.isArray(dataSource)){
            ...
            dataSource.forEach(function(v){
                ...
                //判断是颜色还是尺码，进行渲染
                if(!!v.firstProp){
                    html += '<li style="font-size:'+fontsize+'px" data-no = "' + v.secondProps[0].sku+'">'+v.firstProp+'</li>';
                }else{
                    html += '<li style="font-size:'+fontsize+'px" data-no = "' + v.sku+'">'+v.secondProp+'</li>';
                }
            });
            this.upContains.innerHTML = html;
            upSetPosition(-index*lineHeight,this);
        }
    }
